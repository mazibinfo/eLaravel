@extends('admin_layout')
@section('admin_content')
<ul class="breadcrumb">
	<li>
		<i class="icon-home"></i>
		<a href="index.html">Home</a> 
		<i class="icon-angle-right"></i>
	</li>
	<li><a href="#">All Products</a></li>
</ul>
<p class="alert-success">
	<?php
		$message=Session::get('message');
		if ($message) {
			echo $message;
			Session::put('message',null);
		}
	?>	
</p>
<div class="row-fluid sortable">		
	<div class="box span12">
		<div class="box-header" data-original-title>
			<h2><i class="halflings-icon user"></i><span class="break"></span>All Products</h2>
		</div>
		<div class="box-content">
			<table class="table table-striped table-bordered bootstrap-datatable datatable">
			  <thead>
				  <tr>
					  <th>ID</th>
					  <th>Name</th>
					  <th>Category Name</th>
					  <th>Brand Name</th>
					  <th>Price</th>
					  <th>Size</th>
					  <th>Color</th>
					  <th>Image</th>
					  <th>Short Description</th>
					  <th>Long Description</th>
					  <th>Status</th>
					  <th>Actions</th>
				  </tr>
			  </thead>
			     @foreach($product as $row)
			  <tbody>
				<tr>
					<td>{{$row->id}}</td>
					<td class="center">{{$row->product_name}}</td>
					<td class="center">{{$row->category_name}}</td>
					<td class="center">{{$row->manufacture_name}}</td>
					<td class="center">{{$row->product_price}}</td>
					<td class="center">{{$row->product_size}}</td>
					<td class="center">{{$row->product_color}}</td>
					<td class="center"><img src="{{ URL::to($row->product_image) }}" style="height: 60px; width: 100px;"></td>

					<td class="center">{{$row->product_short_description}}</td>
					<td class="center">{{$row->product_short_description}}</td>
					<td class="center">
						@if($row->publication_status==1)
							<span class="label label-success">Active</span>
						@else
							<span class="label label-danger">Inactive</span>
						@endif
					</td>
					<td class="center">
						@if($row->publication_status==1)
							<a class="btn btn-danger" href="{{ URL::to('/inactive-product/'.$row->id) }}" id="inactive">							
								<i class="halflings-icon white thumbs-down"></i>  
							</a>
						@else
							<a class="btn btn-success" href="{{ URL::to('/active-product/'.$row->id) }}" id="active">										
								<i class="halflings-icon white thumbs-up"></i>  
							</a>
						@endif
							
						<a class="btn btn-info" href="{{ URL::to('/edit-product/'.$row->id) }}">
							<i class="halflings-icon white edit"></i>  
						</a>

						<a class="btn btn-danger" href="{{ URL::to('/delete-product/'.$row->id) }}" id="delete">
							<i class="halflings-icon white trash"></i> 
						</a>
					</td>
				</tr>
				
			  </tbody>
			  @endforeach
		  </table>            
		</div>
	</div><!--/span-->

</div><!--/row-->
@endsection